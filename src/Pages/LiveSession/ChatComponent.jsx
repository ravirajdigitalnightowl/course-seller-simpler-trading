...
// 👇 YAHAN ADD KIYA HAI HELPER FUNCTION 👇
const formatChatTime = (dateString) => {
  ...
  const diffTime = startOfToday.getTime() - startOfMessageDay.getTime();
...
const MessageItem = ({ message, isOwnMessage }) => {
  ...
  return (
    <div className={`flex flex-col max-w-[85%] ${
      ...
      {/* 👇 Updated Name Fallback Here 👇 */}
      <span className="text-xs text-gray-400 mb-1 px-2 whitespace-nowrap">
        {message.name || message.userName || "User"}
      </span>
      ...
      <div className={`px-4 py-2 rounded-2xl text-sm break-words backdrop-blur-sm ${
        ...
        {/* 👇 Updated Timestamp with formatChatTime 👇 */}
        <div className={`text-xs mt-1 text-right whitespace-nowrap ${
          isOwnMessage ? 'text-blue-200' : 'text-gray-400'
        }`}>
          {formatChatTime(message.timestamp || message.at)}
        </div>
      </div>
    </div>
  );
