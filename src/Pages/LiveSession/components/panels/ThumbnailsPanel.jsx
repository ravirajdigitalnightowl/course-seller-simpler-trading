...
const ThumbnailsPanel = ({
  ...
  }, [showWhiteboard, isOverlay, zoomed?.type, handleThumbClick, handleCloseWhiteboard]);

  const streamerThumb = useMemo(() => {
    ...
    return (
      <div className={isOverlay ? "relative group" : ""}>
        <ThumbnailVideo
          ...
          userName="You"
          onClick={() =>
            handleThumbClick({ type: "streamer", stream: mediaStream, userId })
          }
          isZoomed={zoomed?.type === "streamer"}
          videoEnabled={videoEnabled}
          expanded={isOverlay}
  ...
  }, [mediaStream, userId, videoEnabled, zoomed?.type, isOverlay, handleThumbClick]);

  const activeScreenThumb = useMemo(() => {
    if (!activeScreenShare?.stream || !activeScreenVideoStream) return null;
    return (
      <div className={isOverlay ? "relative group" : ""}>
        <ThumbnailVideo
          uid={activeScreenShare.userId || "screen"}
    ...
    activeScreenVideoStream,
    zoomed?.type,
    isOverlay,
    handleThumbClick,
  ]);

  const viewerScreenThumb = useMemo(() => {
    if (!viewerScreenShare?.stream) return null;
    return (
      <div className={isOverlay ? "relative group" : ""}>
        <ThumbnailVideo
          uid={viewerScreenShare.userId}
          ...
          isZoomed={
            ...
            zoomed?.userId === viewerScreenShare.userId
          }
          videoEnabled={true}
          isScreenShare={true}
          expanded={isOverlay}
    ...
    viewerScreenShare?.stream,
    viewerScreenShare?.userId,
    viewerScreenShare?.userName,
    zoomed?.type,
    zoomed?.userId,
    isOverlay,
    handleThumbClick,
  ...
  const viewerCameraThumbs = useMemo(() => {
    return viewerCameraEntries.map(([uid, stream]) => (
      <div key={uid} className={isOverlay ? "relative group" : ""}>
        <ThumbnailVideo
          uid={uid}
          stream={stream}
