...
import ScreenAnnotationWhiteboard from './components/ScreenAnnotationWhiteboard'; // 👈 ✅ YEH LINE ADD KARO
...
import MainStage from "./components/panels/MainStage";
import { toast } from 'react-toastify';
import { useMediaProcessors } from './useMediaProcessors';
// Agar aapne in files ko components folder ke andar banaya hai, toh path kuch aisa hoga:
import StreamHeader from './components/StreamHeader';
import BottomControls from './components/BottomControls';
import StreamSidebar from './components/StreamSidebar';
import StreamLoadingStates from './components/StreamLoadingStates';
import StartResumePromptModal from './components/StartResumePromptModal';
import { useLiveConnection } from './useLiveConnection'; // Naya import
import { FiVideo, FiVideoOff, FiMic, FiMicOff,FiCamera, FiCameraOff,FiEdit3 ,FiUsers, FiMessageSquare,FiChevronRight, FiChevronLeft,
  FiPlay, FiPause, FiStopCircle,FiCheck, FiX,FiLoader,FiLogOut,FiAlertCircle,FiMaximize,FiCircle,FiMonitor,FiRadio,
  FiVolume,FiVolume2,FiVolume1} from 'react-icons/fi';
import { 
...
const [appViewerScreenShare, setAppViewerScreenShare] = useState(null);
...
// LiveSession.jsx ke andar
const [allowedWhiteboardUsers, setAllowedWhiteboardUsers] = useState(() => {
  try {
    // Current session ke basis par localStorage se allowed users nikaalo
    const saved = localStorage.getItem(`whiteboard_permissions_${sessionId || roomCode}`);
    return saved ? JSON.parse(saved) : []; // Array of userIds return karega
  } catch (e) {
    return [];
  }
});
...
// Pagination aur active consumers track karne ke liye
const [viewerVideoPage, setViewerVideoPage] = useState(0);
const activeViewerVideoIdsRef = useRef(new Set());
const connectedAudioSourcesRef = useRef(new Map());
const [pinnedViewerId, setPinnedViewerId] = useState(null);
const VIDEOS_PER_PAGE = 2; // Ek page pe 4 viewer cameras
...
const [showRequests, setShowRequests] = useState(false);
  ...
  const latestResourcesRef = useRef({ mediaStream: null, screenCaptureStream: null, socket: null });
...
const autoZoomedTrackIdRef = useRef(null);
...
const mixedAudioProducerRef = useRef(null);
...
const recordingScreenStreamRef = useRef(null);
const recordingVideoProducerRef = useRef(null);
...
const defaultAnnotationInfo = useMemo(() => ({
  title: session?.title || "Screen Annotation",
  ...
  isStreamer: true
}), [session, roomCode, user, sessionId, token]);


  // Apne states aur refs ke niche ye add karo:
  ...
  const mediaProcessors = useMediaProcessors({
    states: { 
    ...
    setters: {
        ...
        setHandRaisedUsers, setActiveSpeakers, setViewerAudioRequests,setAppViewerScreenShare,
        setViewerVideoRequests, setScreenShareRequests, setActiveViewerAudio,
        setViewerScreenShare, setShowAudioPermissionModal
    },
    refs: {
        speakingAudioContextRef, speakingSourceRef, analyserRef,
        speakingDetectionIntervalRef, localAudioTrackRef, sendTransportRef,
        recordingAudioContextRef, recordingDestinationRef,
...
const {
  ...
  retryCamera,
  initializeWebSocket,
  initializeMedia,         // ✅ Added
  handlePauseProducer,     // ✅ Added
  handleResumeProducer,    // ✅ Added
  handleCloseProducer,
  createConsumer      // ✅ Added
} = useLiveConnection({
  // Core Context
  sessionId, roomCode, token, user, device,
  // Refs
  videoRef, screenRef, socketRef, sendTransportRef, recvTransportRef,
  producers, consumers, transports, remoteVideosRef, 
  isRecordingRef, viewerScreenRef, pendingAudioQueueRef, isMountedRef, // ✅ Missing refs added
  // States
  mediaStream, isMobile, permissionStatus, isMediaInitialized, 
  roomState, producersState, setActiveViewerAudio,
  // Setters
  ...
  setViewerScreenShare, setAppViewerScreenShare, setZoomed, setSpeakingUsers, setHandRaisedUsers,
  setViewerAudioRequests, setScreenShareRequests, setActiveSpeakers,
  setMediaStream, setIceServers, setSession, setViewerVideoRequests,
  setIsPlaying, setIsWebSocketInitialized, setIsRecording, setViewerCameras, // ✅ Missing setters added
  // Helpers & External Processors
  addDebugLog, getSocket, emitSocketEvent, 
  cleanupViewerMedia: mediaProcessors.cleanupViewerMedia,
  cleanupViewerCamera: mediaProcessors.cleanupViewerCamera,
  cleanupScreenShareAudio: mediaProcessors.cleanupScreenShareAudio,
  cleanupOnlyMicrophoneAudio: mediaProcessors.cleanupOnlyMicrophoneAudio,
  cleanupUserOnLeft: mediaProcessors.cleanupUserOnLeft,
  handleAudioConsumer: mediaProcessors.handleAudioConsumer,
  startRecordingTimer // ✅ Added
});
// Ab seedha destructure karke unko file mein waise hi use karo jaise pehle kar rahe the
const {
    startStreamerSpeakingDetection, stopStreamerSpeakingDetection,
    startSpeakingDetection, stopSpeakingDetection, getMixedAudioStream,
    cleanupScreenCapture, cleanupViewerAudio, cleanupViewerCamera,
    cleanupViewerMedia, cleanupScreenShareAudio, cleanupOnlyMicrophoneAudio,
    cleanupUserOnLeft, handleAudioConsumer, playAllAudio, createAndPlayAudioElement
} = mediaProcessors;
...
const thumbnailsCount = (mediaStream ? 1 : 0) + 
                       ...
                       (viewerScreenShare ? 1 : 0)+(appViewerScreenShare ? 1 : 0); // ✅ YEH ADD KARO
...
// ==========================================
// 🧠 MASTER CONTROLLER FOR VIEWER CAMERAS
// ==========================================

// Jaise hi stream active ho (start ya resume), dismiss state ko reset kar dein
useEffect(() => {
  if (roomState.isStreaming && !roomState.isPaused) {
    setIsPromptDismissed(false);
  }
}, [roomState.isStreaming, roomState.isPaused]);
useEffect(() => {
    // Component mount hote hi flag set karein
    sessionStorage.setItem("killMediaOnReturn", "true");
    
    // ... aapka baaki ka LiveSession ka code ...
  }, []);

useEffect(() => {
  return () => {
    console.log("🧹 Streamer leaving page. Cleaning up hardware resources...");
    
    // Ref se ekdam latest values nikaalein
    const { mediaStream: latestMedia, screenCaptureStream: latestScreen, socket: latestSocket } = latestResourcesRef.current;

    if (latestMedia) { 
      latestMedia.getTracks().forEach((track) => {
        track.stop();
        console.log(`🛑 Stopped streamer track: ${track.kind}`);
      });
    }

    if (latestScreen) { 
      latestScreen.getTracks().forEach((track) => {
        track.stop();
        console.log(`🛑 Stopped streamer screen track: ${track.kind}`);
      });
    }

    if (latestSocket) {
      latestSocket.emit("streamer-screen-share-stop", { sessionId: sessionId || roomCode });
    }
  };
}, []);
// Check karein ki modal dikhana hai ya nahi
const showStartResumePrompt = (!roomState.isStreaming || roomState.isPaused) && !isPromptDismissed;
useEffect(() => {
  if (!producersState || !socketRef.current || !recvTransportRef.current) return;

  // Sirf zinda viewer cameras nikal rahe hain
  const allViewerProducers = Array.from(producersState.values()).filter(
    p => p.kind === 'video' && p.source === 'viewer-camera'
  );

  const shouldBeActive = new Set();

  if (thumbnailsExpanded) {
    // Condition A: Agar All Viewers modal khula hai -> Current page ke 3 videos laao
    const VIDEOS_PER_PAGE = 3;
    const startIndex = viewerVideoPage * VIDEOS_PER_PAGE;
    const pageProducers = allViewerProducers.slice(startIndex, startIndex + VIDEOS_PER_PAGE);
    pageProducers.forEach(p => shouldBeActive.add(p.id));
  } else {
    // Condition B: Modal band hai -> Check karo koi Pinned hai kya?
    let activeProducer = null;
    
    if (pinnedViewerId) {
      // 📌 Jo viewer select (Pin) hua tha, uski stream zinda rakho!
      activeProducer = allViewerProducers.find(p => p.userId === pinnedViewerId);
    }
    
    // Agar koi Pin nahi hai, par auto-zoom wagera se koi viewer bada hai toh use dikhao
    if (!activeProducer && zoomed?.type === "viewer" && zoomed?.userId) {
      activeProducer = allViewerProducers.find(p => p.userId === zoomed.userId);
    }

    // Us ek viewer camera ko consume list me daal do
    if (activeProducer) {
      shouldBeActive.add(activeProducer.id);
    }
  }

  // 2. 🧹 CLEANUP
  activeViewerVideoIdsRef.current.forEach(producerId => {
    if (!shouldBeActive.has(producerId)) {
      const consumer = consumers.current.get(producerId);
      if (consumer) {
        try { consumer.close(); } catch(e) {}
        consumers.current.delete(producerId);
        emitSocketEvent('consumer-close', { consumerId: consumer.id });
      }

      const producerInfo = producersState.get(producerId);
      if (producerInfo) {
        setViewerCameras(prev => {
          const newMap = new Map(prev);
          const stream = newMap.get(producerInfo.userId);
          if (stream) stream.getTracks().forEach(t => t.stop());
          newMap.delete(producerInfo.userId);
          return newMap;
        });
      }
    }
  ...
  // 3. 📥 CONSUME NEW
  shouldBeActive.forEach(producerId => {
    if (!activeViewerVideoIdsRef.current.has(producerId)) {
      const producer = producersState.get(producerId);
      if (producer) {
        createConsumer(sessionId || roomCode, producer.id, producer.kind);
      }
    }
  });

  activeViewerVideoIdsRef.current = new Set(shouldBeActive);

// 👇 Yahan dependencies me pinnedViewerId hona zaroori hai!
}, [producersState, viewerVideoPage, pinnedViewerId, zoomed, thumbnailsExpanded, sessionId, roomCode]);
...
// ==========================================
// AUTO-MIXER USE-EFFECT (For Dynamic Audio)
// ==========================================
useEffect(() => {
  // Agar recording on nahi hai ya mixer setup nahi hai, toh kuch mat karo
  if (!isRecording || !recordingAudioContextRef.current || !recordingDestinationRef.current) return;

  const audioCtx = recordingAudioContextRef.current;
  const dest = recordingDestinationRef.current;
  const currentActiveTrackIds = new Set();

  // Ek helper function jo naye track ko mixer mein jodta hai
  const addTrackToMixer = (track) => {
    if (!track || track.readyState === 'ended') return;
    currentActiveTrackIds.add(track.id);
    
    if (!connectedAudioSourcesRef.current) {
      connectedAudioSourcesRef.current = new Map();
    }
    
    // Agar track pehle se nahi juda hai, toh jod do
    if (!connectedAudioSourcesRef.current.has(track.id)) {
      try {
        const stream = new MediaStream([track]);
        const source = audioCtx.createMediaStreamSource(stream);
        source.connect(dest); // Aawaz ko recording destination mein bhejo
        connectedAudioSourcesRef.current.set(track.id, source);
        console.log(`🎙️ Added dynamic audio track to mixer: ${track.id}`);
      } catch (e) {
        console.warn("Error adding track to mixer", e);
      }
    }
  };

  // 1. Streamer ka Apna Mic daalo
  mediaStream?.getAudioTracks()?.forEach(track => addTrackToMixer(track));

  // 2. Viewers ka Audio (Mic + Screen Audio) daalo 
  // Tumhare code mein ye 'viewerAudios' Map mein store hota hai
  viewerAudios?.forEach((stream) => {
    stream?.getAudioTracks()?.forEach(track => addTrackToMixer(track));
  });

  // 3. Cleanup: Jo log meeting chhod gaye ya mic off kar diya, unki aawaz mixer se hatao
  if (connectedAudioSourcesRef.current) {
    connectedAudioSourcesRef.current.forEach((source, trackId) => {
      if (!currentActiveTrackIds.has(trackId)) {
        try { source.disconnect(); } catch (e) {}
        connectedAudioSourcesRef.current.delete(trackId);
        console.log(`🛑 Removed audio track from mixer: ${trackId}`);
      }
    });
  }

}, [isRecording, mediaStream, viewerAudios]); // 👈 Ab yahan tumhara actual state variable 'viewerAudios' hai
useEffect(() => {
...
useEffect(() => {
  latestResourcesRef.current = { mediaStream, screenCaptureStream, socket };
}, [mediaStream, screenCaptureStream, socket]);
...
const startRecording = async () => {
  try {
    ...
    if (!socket || !sessionId) throw new Error("Socket/sessionId missing");

    // ==========================================
    // 1. DEDICATED SCREEN CAPTURE FOR RECORDING
    // ==========================================
    const recordScreenStream = await navigator.mediaDevices.getDisplayMedia({
      video: {
        frameRate: { ideal: 30, max: 30 },
        width: { ideal: 1920, max: 1920 },
        height: { ideal: 1080, max: 1080 }
      },
      audio: true // 👈 YAHAN TRUE KIYA TAAKI SYSTEM/TAB AUDIO BHI AAYE
    });

    // Agar user browser ke "Stop sharing" button pe click kar de
    const videoTrack = recordScreenStream.getVideoTracks()[0];
    videoTrack.onended = () => {
      addDebugLog("🛑 Dedicated recording screen share ended by user");
      if (isRecordingRef.current) {
        stopRecording(); // Stop backend recording as well
      }
    };

    // Save stream in ref so we can stop it later
    recordingScreenStreamRef.current = recordScreenStream;

    // Produce this dedicated video track to backend
    if (recordingVideoProducerRef?.current) {
      try { recordingVideoProducerRef.current.close(); } catch {}
    }
    // const videoProducer = await sendTransportRef.current.produce({
    //   track: videoTrack,
    //   appData: { source: "recording-video" }, // 👈 Naya tag backend ke liye
    //   codecOptions: { videoGoogleStartBitrate: 1000 }
    // });

    // startRecording function ke andar videoProducer setup:
const videoProducer = await sendTransportRef.current.produce({
  track: videoTrack,
  appData: { source: "recording-video" },
  encodings: [
    { 
      maxBitrate: 1200000,      // 1.2 Mbps (Exact as requested)
      minBitrate: 600000,       // Fallback control: 600kbps se niche nahi girega
      maxFramerate: 30,         // 30 FPS
      priority: 'high',         // Network congestion mein isko priority milegi
      networkPriority: 'high'
    }
  ],
  codecOptions: { 
    videoGoogleStartBitrate: 1000, // 1 Mbps se start hoga
    videoGoogleMinBitrate: 600     // Engine ko force karega low quality avoid karne ke liye
  }
});
    recordingVideoProducerRef.current = videoProducer;


    // ==========================================
    // 2. SETUP CUSTOM AUDIO MIXER 
    // (Streamer Mic + Dedicated Screen Audio)
    // ==========================================
    ...
    // Naya Master Destination banayenge jahan sab mix hoga
    const dest = audioCtx.createMediaStreamDestination();
    recordingDestinationRef.current = dest;

    // A. Streamer ka apna Mic add karo
    let currentMicStream = mediaStream?.getAudioTracks?.()?.length > 0 ? mediaStream : null;
    if (currentMicStream) {
      const micSource = audioCtx.createMediaStreamSource(currentMicStream);
      micSource.connect(dest);
    }

    // B. Dedicated Screen ka System Audio add karo
    if (recordScreenStream.getAudioTracks().length > 0) {
      const screenSource = audioCtx.createMediaStreamSource(recordScreenStream);
      const gainNode = audioCtx.createGain();
      gainNode.gain.value = 0.5; // Screen ki aawaz ko thoda balance rakho taaki Mic clear rahe
      screenSource.connect(gainNode).connect(dest);
    }

    // Final mixed track nikal lo
    const mixedAudioTrack = dest.stream.getAudioTracks()[0];

    // ==========================================
    // 3. PRODUCE MIXED AUDIO
    // ==========================================
    if (mixedAudioTrack) {
      if (mixedAudioProducerRef?.current) {
        try { mixedAudioProducerRef.current.close(); } catch {}
      }
      const mixedProducer = await sendTransportRef.current.produce({
        track: mixedAudioTrack,
        appData: { source: "mixed-audio" }, // 👈 Backend isko dhoondh lega
      });
      mixedAudioProducerRef.current = mixedProducer;
    }

    // Wait slightly to let producers initialize on backend
    await new Promise(resolve => setTimeout(resolve, 800));

    // ==========================================
    // 4. TRIGGER BACKEND RECORDING
    // ==========================================
    socket.emit("server-recording-start", { sessionId, userId: user?.id });
    ...
    // Agar user ne screen share permission cancel kar di
    if (error.name === 'NotAllowedError') {
      toast.error("Screen sharing permission denied for recording");
    } else {
      toast.error("Failed to start recording");
    }

    // Cleanup midway failure
    if (recordingScreenStreamRef.current) {
      recordingScreenStreamRef.current.getTracks().forEach(t => t.stop());
...
const startRecordingFromScreenShare = async (screenShareStream) => {
  try {
    setIsRecordingLoading(true);
    if (isRecording === true || isRecordingRef?.current === true) return;
    ...
    socket.emit("server-recording-start", { sessionId, userId: user?.id });
    setIsRecording(true);
    if (isRecordingRef?.current !== undefined) isRecordingRef.current = true;
    startRecordingTimer();
    ...
    toast.error("Failed to start recording");
    setIsRecording(false);
    if (isRecordingRef?.current !== undefined) isRecordingRef.current = false;
  } finally {
    setIsRecordingLoading(false);
  }
};
...
const startScreenShareForParticipants = async (screenStream) => {
  ...
  try {
    ...
    try {
      ...
      // });
    // startScreenShareForParticipants function ke andar:
    
    const vp8Codec = device.rtpCapabilities.codecs.find(
      (c) => c.mimeType.toLowerCase() === 'video/hvp8'
    );
  ...
  encodings: [
    { 
      maxBitrate: 2000000,      // 2 Mbps for High Quality 30fps
      minBitrate: 800000,       // Fallback control: 800kbps safe limit for text
      maxFramerate: 30,         // 30 FPS
      priority: 'very-high'     // Live view ko priority
    }
  ],
  codecOptions: { 
...
const stopScreenShare = useCallback(async () => {
  ...
  // 1. Tell backend to stop forwarding
  if (socket && sessionId) {
  ...
  // 2. Close Producers
  const isShareScreenSource = (src = "") =>
    src === "screen" || src === "screen-audio" ||
    src === "streamer-screen" || src === "streamer-screen-audio";

  const toDelete = [];
  producers.current?.forEach?.((producer, id) => {
    if (!isShareScreenSource(producer?.appData?.source || "")) return;
    try {
      producer.close();
      toDelete.push(id);
    } catch (err) {}
  ...
  toDelete.forEach((id) => producers.current.delete(id));
  
  if (toDelete.length) {
    setProducersState((prev) => {
      const m = new Map(prev);
      toDelete.forEach((id) => m.delete(id));
      return m;
    });
  }

  // 3. Stop Local Tracks
  const shareObj = activeScreenShareRef?.current || activeScreenShare;
  if (shareObj?.source === "streamer" && shareObj?.stream) {
    try {
      shareObj.stream.getTracks?.().forEach((t) => {
        try { t.stop(); } catch {}
      });
    } catch (e) {}
  }
  ...
  setActiveScreenShare((prev) => (prev?.source === "streamer" ? null : prev));
  
  // 5. Clean capture completely
  cleanupScreenCapture(true);

}, [socket, sessionId, roomCode, user, activeScreenShare, setProducersState, setActiveScreenShare, setZoomed]);

const stopRecording = useCallback(async () => {
  if (isRecordingStopping) return;
  const sid = sessionId || roomCode;

  try {
    addDebugLog("⏹️ Stopping SERVER recording...");
    setIsRecordingStopping(true);
    isRecordingRef.current = false;

    if (!socket || !sid) throw new Error("Socket/sessionId missing");

    await new Promise((resolve) => {
      socket.emit("server-recording-stop", { sessionId: sid }, (res) => resolve(res));
    });

    // ==========================================
    // 1. Dedicated Recording Producer Cleanup (NAYA)
    // ==========================================
    try {
      if (recordingVideoProducerRef?.current) {
        try { recordingVideoProducerRef.current.close(); } catch {}
        recordingVideoProducerRef.current = null;
        addDebugLog("🧹 Dedicated recording video producer closed");
      }
    } catch {}

    // ==========================================
    // 2. Dedicated Screen Capture Stream Cleanup (NAYA)
    // ==========================================
    try {
      if (recordingScreenStreamRef?.current) {
        recordingScreenStreamRef.current.getTracks().forEach(t => {
          try { t.stop(); } catch {}
        });
        recordingScreenStreamRef.current = null;
        addDebugLog("🧹 Dedicated recording screen stream killed");
      }
    } catch {}

    // ==========================================
    // 3. Mixed audio producer band karo
    // ==========================================
    try {
      if (mixedAudioProducerRef?.current) {
        try { mixedAudioProducerRef.current.close(); } catch {}
        mixedAudioProducerRef.current = null;
      }
    } catch {}
...
}, [socket, sessionId, roomCode, isRecordingStopping, activeScreenShare, screenCaptureActive, cleanupScreenCapture, stopRecordingTimer]);


const handleWhiteboardPermission = useCallback((targetUserId, canDraw) => {
  // Socket emit as usual
  emitSocketEvent('update_whiteboard_permission', {
    sessionId: sessionId || roomCode,
    targetUserId: targetUserId,
    canDraw: canDraw
  });
  
  // State aur LocalStorage update karein
  setAllowedWhiteboardUsers(prev => {
    let updatedList;
    if (canDraw) {
      updatedList = [...new Set([...prev, targetUserId])]; // Add user
    } else {
      updatedList = prev.filter(id => id !== targetUserId); // Remove user
    }
    
    // Local storage me save karein taaki reload par data persist rahe
    localStorage.setItem(`whiteboard_permissions_${sessionId || roomCode}`, JSON.stringify(updatedList));
    return updatedList;
  });

  toast.success(`Whiteboard permission ${canDraw ? 'granted' : 'revoked'}`);
  addDebugLog(`✏️ Updated whiteboard permission for ${targetUserId} to ${canDraw}`);
}, [sessionId, roomCode, emitSocketEvent]);
...
const handleStopViewerScreenShare = useCallback((targetUserId) => {
    // 1. Server ko force-stop ka command bhejo
    emitSocketEvent('screen-share-force-stop', {
      sessionId: sessionId || roomCode,
      targetUserId
    });
    
    // 2. Web Viewer ka state locally clear karo
    setViewerScreenShare(prev => {
      if (prev?.userId?.toString() === targetUserId?.toString()) return null;
      return prev;
    });

    // 3. App Viewer ka state locally clear karo
    setAppViewerScreenShare(prev => {
      if (prev?.userId?.toString() === targetUserId?.toString()) return null;
      return prev;
    });

    // 4. Agar Streamer usko bada (zoom) karke dekh raha tha, toh zoom hatao
    setZoomed(prev => {
      if (
        prev && 
        (prev.type === "viewer-screen" || prev.type === "app-viewer-screen") && 
        prev.userId?.toString() === targetUserId?.toString()
      ) {
        return null;
      }
      return prev;
    ...
    // 5. App screen share ke audio tags ko DOM se clean karo (Ghost audio fix)
    if (typeof cleanupScreenShareAudio === 'function') {
      cleanupScreenShareAudio(targetUserId);
    }
    
    addDebugLog(`Stopped screen share for user: ${targetUserId}`);
    toast.success("Viewer screen share stopped"); // Optional: Streamer ko feedback dene ke liye
    
  }, [sessionId, roomCode, setViewerScreenShare, setAppViewerScreenShare, setZoomed, cleanupScreenShareAudio, emitSocketEvent]);
...
const endStreaming = async () => {
  // 1. Pehle user se confirm karein
  if (window.confirm('Are you sure you want to end the session for all participants?')) {
    
    // 2. Agar recording chal rahi hai, toh modal dikhayein aur yahin ruk jayein
    if (isRecording) {
      setShowEndSessionRecordingModal(true);
      return;   
    }

    // 3. Agar recording nahi hai, toh seedha session end karein
    actuallyEndSession();
  }
};

const actuallyEndSession = () => {
  // 4. Sabhi participants ke liye session end karne ka event bhejein
  emitSocketEvent('streamer_control', { 
    sessionId: sessionId || roomCode, 
    status: 'ENDED', 
  ...
  // 5. Server ko session close command bhejein
  if (socket) {
    socket.emit("end-session", { sessionId: sessionId || roomCode });
  }

  // 6. Dashboard par redirect karein
  navigate("/dashboard");
};
...
const sendMessage = useCallback(async (text = '', file = null) => {
  ...
  useEffect(() => {
  const trackId = mediaStream?.getVideoTracks?.()?.[0]?.id;
  if (!trackId) return;

  // ✅ already auto-zoomed for this track -> skip
  if (autoZoomedTrackIdRef.current === trackId) return;

// ✅ only when nothing else is taking main stage
  if (activeScreenShare || viewerScreenShare || appViewerScreenShare || showWhiteboard) return; // 👈 appViewerScreenShare add kiya

  // ✅ If user already zoomed something, don't override
  if (zoomed) return;

  // ✅ mark & auto zoom (same as thumbnail click behavior)
  autoZoomedTrackIdRef.current = trackId;

  const videoOnly = getVideoOnlyStream(mediaStream);
  setZoomed({ type: "streamer", stream: videoOnly, userId: user?.id });

  // optional: ensure play attempt (desktop mostly no issue)
  setTimeout(() => {
...
// Render se thoda upar add karo:
const totalViewerProducersCount = Array.from(producersState.values()).filter(p => p.kind === 'video' && p.source === 'viewer-camera').length;
  // Mobile main render
  if (isMobile) {
...
if (
    ...
    (permissionStatus === 'denied' && !isConnected) ||
    (!isConnected || !session)
  ) {
    return (
      <StreamLoadingStates
  ...
  return (
    <div className="flex flex-col h-[115vh] bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white overflow-hidden">
      ...
      <StreamHeader 
        ...
        isConnected={isConnected}
        pendingRequestsCount={viewerAudioRequests.length + screenShareRequests.length + viewerVideoRequests.length}
      ...
      <div className="flex-1 overflow-hidden relative">
        {/* Thumbnails Full Screen Overlay */}
        {thumbnailsExpanded && (
          <div className="absolute inset-0 bg-black/95 backdrop-blur-sm z-40 flex flex-col">
            <div className="p-4 border-b border-gray-700 flex items-center justify-between bg-gray-800/90">
              ...
              <button onClick={() => setThumbnailsExpanded(false)} className="p-2 hover:bg-gray-700 rounded-lg">
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <ThumbnailsPanel
                ...
                // 👇 YEH 3 NAYE PROPS ADD KARNE HAIN 👇
                viewerVideoPage={viewerVideoPage}
                setViewerVideoPage={setViewerVideoPage}
                totalViewerProducersCount={totalViewerProducersCount}
                setPinnedViewerId={setPinnedViewerId}

              />
            </div>
            <div className="p-4 border-t border-gray-700 bg-gray-800/90 flex justify-between items-center">
              <div className="text-sm text-gray-400">Click on any stream to zoom into it</div>
              <button onClick={() => setThumbnailsExpanded(false)} className="px-6 py-2 bg-blue-600 rounded-lg">
                Back to Stream
              </button>
            </div>
          </div>
        ...
        <div className="flex h-full">
          ...
          <div className="flex-1 flex flex-col min-h-0">
            <MainStage
              ...
              mediaStream={mediaStream}
              videoRef={videoRef}
              appViewerScreenShare={appViewerScreenShare}
              screenRef={screenRef}
              zoomedVideoRef={zoomedVideoRef}
              participants={participants}
              activeScreenShare={activeScreenShare}
              viewerScreenShare={viewerScreenShare}
              showWhiteboard={showWhiteboard}
              whiteboardSessionInfo={whiteboardSessionInfo || defaultAnnotationInfo} // ✅ Ab ye hamesha data pass karega
              ScreenAnnotationWhiteboard={ScreenAnnotationWhiteboard}
              StreamerWhiteboard={StreamerWhiteboard}
              handleCloseWhiteboard={handleCloseWhiteboard}
              isPlaying={isPlaying}
              isLoading={isLoading}
              isInitializing={isInitializing}
              ...
              renderSidebarToggleButton={() => (
                <div className="absolute right-4 top-4 z-50">
                  <button
                    ...
                    className="bg-gray-800/80 hover:bg-gray-700/80 p-3 rounded-lg shadow-lg"
                    title={sidebarCollapsed ? "Show Sidebar" : "Hide Sidebar"}
                  >
                    {sidebarCollapsed ? <FiChevronLeft className="h-5 w-5 text-blue-400" /> : <FiChevronRight className="h-5 w-5 text-blue-400" />}
                  </button>
                </div>
              )}
            />
          ...
          <div className="w-48 xl:w-56 flex flex-col bg-gray-800/80 border-l border-gray-600 z-30 h-full">
            <div className="p-4 border-b border-gray-600 flex items-center justify-between">
              ...
              {thumbnailsCount > 0 && (
                <button onClick={() => setThumbnailsExpanded(true)} className="p-1 hover:bg-gray-700 rounded">
          ...
          {!sidebarCollapsed && (
            <StreamSidebar 
              ...
              lowerHandForUser={lowerHandForUser}
              handleMuteViewerAudio={handleMuteViewerAudio}
              emitSocketEvent={emitSocketEvent}
              handleWhiteboardPermission={handleWhiteboardPermission}
              allowedWhiteboardUsers={allowedWhiteboardUsers} // 👈 NAYA PROP
              addDebugLog={addDebugLog}
            />
          )}
        </div>
      ...
      <AudioPermissionModal isOpen={showAudioPermissionModal} onEnableAudio={handleEnableAudio} />
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-[9999]">
          <div className="bg-gray-800 p-8 rounded-2xl border border-blue-500/30 text-center max-w-md shadow-2xl mx-4">
            ...
            <p className="text-gray-300 mb-8 text-sm">
              To prevent echo during screen recording, we need to sync your audio context. Please click the button below.
            </p>
            <button
              onClick={handleEnableAudio}
              className="w-full py-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-xl font-bold flex items-center justify-center space-x-2"
            >
      ...
      {/* Panels and Modals */}
      <ViewerAudioRequestsPanel viewerAudioRequests={viewerAudioRequests} handleViewerAudioResponse={handleViewerAudioResponse} />
      <ScreenShareRequestsPanel screenShareRequests={screenShareRequests} handleScreenShareResponse={handleScreenShareResponse} />
      <ActiveViewerAudioPanel activeViewerAudio={activeViewerAudio} handleMuteViewerAudio={handleMuteViewerAudio} />
      <ActiveScreenSharesPanel activeScreenShare={viewerScreenShare || appViewerScreenShare} handleStopViewerScreenShare={handleStopViewerScreenShare} />
      <ParticipantsModal
        ...
        handleWhiteboardPermission={handleWhiteboardPermission}
      />
      <ViewerVideoRequestsPanel viewerVideoRequests={viewerVideoRequests} handleViewerVideoResponse={handleViewerVideoResponse} />

      {/* Remove Audio Confirmation Modal */}
      {audioRemoveTarget && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-2xl shadow-2xl max-w-sm text-gray-900">
            <div className="flex items-center space-x-3 mb-4">
              <FiAlertCircle className="h-6 w-6 text-red-500" />
              <h3 className="text-lg font-semibold">Remove Audio?</h3>
            </div>
            <p className="mb-6">Do you want to remove audio for <strong>{audioRemoveTarget.name || audioRemoveTarget.userId}</strong>?</p>
            <div className="flex justify-end space-x-3">
              <button onClick={() => setAudioRemoveTarget(null)} className="px-4 py-2 bg-gray-300 rounded-lg">Cancel</button>
              <button onClick={confirmRemoveAudio} className="px-4 py-2 bg-red-600 text-white rounded-lg">Remove Audio</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showEndSessionRecordingModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-[420px] text-center shadow-xl">
            <h2 className="text-lg font-bold text-gray-800">🎬 Recording is still ON</h2>
            <p className="mt-3 text-gray-600">Do you want to save this recording before ending the session?</p>
            <div className="mt-5 flex justify-center gap-4">
              <button onClick={async () => { await stopRecording(); setShowEndSessionRecordingModal(false); actuallyEndSession(); }} className="bg-green-600 text-white px-5 py-2 rounded-lg">
                ✅ Save & End
              </button>
              <button onClick={() => { setShowEndSessionRecordingModal(false); actuallyEndSession(); }} className="bg-red-600 text-white px-5 py-2 rounded-lg">
                ❌ End Without Saving
              </button>
            </div>
          </div>
      ...
      <BottomControls 
        ...
        handleStreamerScreenShareClick={handleStreamerScreenShareClick}
        isRecording={isRecording}
        ...
        endStreaming={endStreaming}
      />
      {/* Start / Resume Prompt Modal Component */}
{/* ✅ UPDATED Start / Resume Prompt Modal Component */}
<StartResumePromptModal 
  showStartResumePrompt={showStartResumePrompt}
  onDismiss={() => {
    setIsPromptDismissed(true); // Modal hide karega
    setUserInteracted(true); // ✅ Browser ko batayega ki user active hai (Autoplay policy bypass)
    
    // ✅ Forcefully local video ko unlock karega bina stream tode
    if (videoRef.current && videoRef.current.paused) {
      videoRef.current.play().catch(e => console.log("Video unlock pending"));
    }
  }}
  roomState={roomState}
  resumeStreaming={resumeStreaming}
/>
      ...
      {/* Quick Status Bar Below Controls */}
      <div className="mt-2 mb-2 mx-auto max-w-2xl text-xs text-center text-gray-400">
        {(screenCaptureActive || activeScreenShare) && (
          <div className="flex items-center justify-center space-x-2">
            <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
            <span className="font-medium text-gray-300">
              {screenCaptureActive && !activeScreenShare ? 'Capture Ready' : activeScreenShare && isRecording ? 'Sharing + Recording' : 'Sharing Active'}
            </span>
          </div>
        )}
      </div>
...
// isme return full optimised hai
// import React, { useState, useEffect, useRef, useCallback,useMemo } from 'react';
// import { useParams, useNavigate } from 'react-router-dom';
// import { useAuth } from '../../contexts/AuthContext';
// import { io } from 'socket.io-client';
...
// const dummyScreenProducerRef = useRef(null);
// const dummyCamProducerRef = useRef(null);
// const dummyScreenTrackRef = useRef(null);
// const dummyCamTrackRef = useRef(null);
...
//         speakingAudioContextRef, speakingSourceRef, analyserRef,
//         speakingDetectionIntervalRef, localAudioTrackRef, sendTransportRef,
//         dummyScreenProducerRef, dummyScreenTrackRef, dummyCamProducerRef,
//         dummyCamTrackRef, recordingAudioContextRef, recordingDestinationRef,
//         viewerAudiosRef, audioElementsRef, pendingAudioQueueRef,
//         isRecordingRef, screenCaptureStreamRef, consumers, remoteVideosRef,
//         userInteractedRef, viewerScreenRef
//     },
...
// const {
//     startStreamerSpeakingDetection, stopStreamerSpeakingDetection,
//     startSpeakingDetection, stopSpeakingDetection, createBlankVideoTrack,
//     startDummyProducer, stopDummyProducer, getMixedAudioStream,
//     cleanupScreenCapture, cleanupViewerAudio, cleanupViewerCamera,
//     cleanupViewerMedia, cleanupScreenShareAudio, cleanupOnlyMicrophoneAudio,
//     cleanupUserOnLeft, handleAudioConsumer, playAllAudio, createAndPlayAudioElement
// } = mediaProcessors;
...
//     setIsRecordingLoading(true);
//     isRecordingStartingRef.current = true;
//     addDebugLog(`🎬 Starting Recording (Screen & Audio Only Mode)`);

//     if (!socket || !sessionId) throw new Error("Socket/sessionId missing");

//     const shareStream = activeScreenShareRef.current?.stream;
//     let currentMicStream = mediaStream?.getAudioTracks?.()?.length > 0 ? mediaStream : null;

//     const mixedAudioTrack = getMixedAudioStream(shareStream, currentMicStream);
//     try {
//       if (recordingAudioContextRef.current?.state === "suspended") {
//         await recordingAudioContextRef.current.resume();
//       }
...
//     // Start Dummy Screen Track agar actual screen off hai
//     if (!activeScreenShareRef.current) {
//       await startDummyProducer('dummy-screen');
//     }
...
//     setIsRecording(false);
//     if (isRecordingRef?.current !== undefined) isRecordingRef.current = false;
//     stopDummyProducer('dummy-screen');
//   } finally {
//     isRecordingStartingRef.current = false;
//     setIsRecordingLoading(false);
//   }
...
//       // 🔥 Screen share ON → Auto recording start/continue
//       if (!isRecordingRef.current) {
//         // Pehli baar — naya clip shuru karo
//         addDebugLog("🎬 Screen share ON → Starting new recording clip...");

//         // Producer backend pe register hone do
//         await new Promise((r) => setTimeout(r, 800));

//         socket.emit("server-recording-start", { sessionId: sessionId || roomCode, userId: user?.id }, (res) => {
//           if (res?.ok) {
//             addDebugLog(`✅ Recording clip started: ${res?.outPath || "n/a"}`);
//           } else {
//             addDebugLog(`⚠️ Recording start issue: ${res?.error || "unknown"}`);
//           }
//         });
...
//         isRecordingRef.current = true;
//         startRecordingTimer();
//         addDebugLog("✅ Recording started automatically on screen share");

//       } else {
//         // Recording chal rahi thi, screen share dobara start hua — consumer swap
//         addDebugLog("🔄 Screen share restarted → Swapping consumers (same clip continues)");

//         if (dummyScreenProducerRef?.current) {
//           stopDummyProducer("dummy-screen");
//           addDebugLog("🎭 Dummy screen stopped");
//         }

//         await new Promise((r) => setTimeout(r, 500));
//         await new Promise((r) => setTimeout(r, 800));

//         socket.emit("server-recording-update", { sessionId: sessionId || roomCode });
//         addDebugLog("🔄 Consumer swap sent to server");

//         if (audioTrack && recordingAudioContextRef?.current && recordingDestinationRef?.current) {
//           try {
//             const newSource = recordingAudioContextRef.current.createMediaStreamSource(screenStream);
...
//             window.recordingAudioSources.set("streamer-screen-audio", { source: newSource, gainNode, type: "mixer" });
//             addDebugLog("🎙️ Screen audio injected into mixer");
//           } catch (e) {
//             addDebugLog(`⚠️ Audio mixer injection failed: ${e.message}`);
//           }
//         }
//       }
//     }
...
//   // 🔥 Screen share OFF → Clip save karo
//   if (socket && isRecordingRef.current) {
//     addDebugLog("📦 Screen share stopped → Saving clip (recording continues for next share)...");

//     // Producers backend se remove hone do
//     await new Promise((r) => setTimeout(r, 600));

//     socket.emit("server-recording-stop", { sessionId: sessionId || roomCode }, (res) => {
//       if (res?.ok) {
//         addDebugLog(`✅ Clip saved: ${res?.outPath || "n/a"} (total clips: ${res?.clipsCount ?? "?"})`);
//       } else {
//         addDebugLog(`⚠️ Clip save issue: ${res?.error || "unknown"}`);
//       }
//     });

//     // ✅ isRecordingRef reset karo taaki next screen share pe NAYA clip start ho
//     // (setIsRecording mat karo — UI button same rahega)
//     isRecordingRef.current = false;
//     addDebugLog("📦 Clip saved. isRecordingRef reset for next screen share.");
//   }
...
//     setIsRecording(false);
//     setRecordingSource(null);
//     toast.success("⏹️ Server Recording stopped!");
//   } catch (error) {
//     console.error("stopRecording error:", error);
//     toast.error(error?.message || "Failed to stop recording");
//     setIsRecording(false);
...
//   // ✅ FIX 1: Recording start karo viewer screen ke liye
//   setTimeout(async () => {
//     const sock = socketRef.current;
//     if (!sock) return;

//     if (!isRecordingRef.current) {
//       // Pehli baar recording shuru karo
//       sock.emit("server-recording-start", 
...
//             isRecordingRef.current = true;
//             startRecordingTimer?.();
//             addDebugLog(`✅ Viewer screen recording started: ${res?.outPath || "n/a"}`);
//           }
//         }
//       );
//     } else {
//       // Recording chal rahi hai — sirf consumer swap karo
//       sock.emit("server-recording-update", { sessionId: sessionId || roomCode });
//       addDebugLog("🔄 Consumer swap for viewer screen share");
//     }
//   }, 800);

// }, [addDebugLog, sessionId, roomCode]);
...
//   // ✅ FIXED: Viewer screen share band hui → Clip save karo
//   if (isRecordingRef.current && socket) {
//     addDebugLog("🛑 Viewer screen stopped → Saving recording clip...");
...
//       (res) => {
//         if (res?.ok) {
//           addDebugLog(`✅ Viewer screen clip saved: ${res?.outPath || "n/a"} (clips: ${res?.clipsCount ?? "?"})`);
//         } else {
//           addDebugLog(`⚠️ Viewer screen clip save issue: ${res?.error || "unknown"}`);
//         }
//       }
//     );

//     // ✅ FIX 1: sirf isRecordingRef reset karo
//     // setIsRecording(false) aur stopRecordingTimer() NAHI — 
//     // streamer ki recording UI aur timer affect nahi honi chahiye
//     isRecordingRef.current = false;
//     addDebugLog("📦 Viewer screen clip saved. isRecordingRef reset for next share.");
//   }

//   addDebugLog(`✅ Viewer screen share cleared for user: ${data.userId}`);
// }, [viewerScreenShare, socket, sessionId, roomCode, isRecordingRef]);
//   const handleStopViewerScreenShare = useCallback((targetUserId) => {
//     emitSocketEvent('screen-share-force-stop', {
//       sessionId: sessionId || roomCode,
//       targetUserId
...
//   addDebugLog(`📜 Chat history received: ${data.messages?.length} messages`);
...
//   // ✅ only when nothing else is taking main stage
//   if (activeScreenShare || viewerScreenShare || showWhiteboard) return;
...
//       <ScreenShareRequestsPanel screenShareRequests={screenShareRequests} handleScreenShareResponse={handleScreenShareResponse} />
//       <ActiveViewerAudioPanel activeViewerAudio={activeViewerAudio} handleMuteViewerAudio={handleMuteViewerAudio} />
//       <ActiveScreenSharesPanel activeScreenShare={viewerScreenShare} handleStopViewerScreenShare={handleStopViewerScreenShare} />
//       <ParticipantsModal
//         showParticipantsModal={showParticipantsModal}
//         setShowParticipantsModal={setShowParticipantsModal}
//         participants={participants}
...
// isme 1000 line ka code ko alag kiya gya aur uske bavjud v sahi kaam kr rha hai 
// import React, { useState, useEffect, useRef, useCallback,useMemo } from 'react';
// import { useParams, useNavigate } from 'react-router-dom';
// import { useAuth } from '../../contexts/AuthContext';
// import { io } from 'socket.io-client';
...
// const createBlankVideoTrack = ({ width = 1280, height = 720, fps = 30 } = {}) => {
...
//   // 🔥 FORCE MOTION to prevent WebRTC frame dropping
//   const interval = setInterval(() => {
//     ctx.fillStyle = "black";
//     ctx.fillRect(0, 0, width, height);
//     ctx.fillStyle = toggle ? "#010101" : "black";
...
//   const track = stream.getVideoTracks()[0];
//   track.onended = () => clearInterval(interval);
//   track.stopInterval = () => clearInterval(interval); 
//   return track;
// };

// const startDummyProducer = async (sourceType) => {
//   if (!sendTransportRef.current) return null;
//   try {
//     const isScreen = sourceType === 'dummy-screen';
//     const track = createBlankVideoTrack({ width: isScreen ? 1280 : 640, height: isScreen ? 720 : 360 });
//     const producer = await sendTransportRef.current.produce({
//       track,
//       encodings: [{ maxBitrate: 300_000 }], 
//       appData: { source: sourceType, userId: user?.id, dummy: true }
//     });
//     if (isScreen) {
//       dummyScreenProducerRef.current = producer;
//       dummyScreenTrackRef.current = track;
//     } else {
//       dummyCamProducerRef.current = producer;
//       dummyCamTrackRef.current = track;
//     }
//     return producer;
//   } catch (e) {
//     console.error(`Failed to start ${sourceType}:`, e);
//     return null;
//   }
// };

// const stopDummyProducer = (sourceType) => {
//   const isScreen = sourceType === 'dummy-screen';
//   const producerRef = isScreen ? dummyScreenProducerRef : dummyCamProducerRef;
//   const trackRef = isScreen ? dummyScreenTrackRef : dummyCamTrackRef;
