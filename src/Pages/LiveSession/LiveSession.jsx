...
const [isRecordingStopping, setIsRecordingStopping] = useState(false); // ✅ नया state add करें
  ...
  // Add these states
const [thumbnailsPosition, setThumbnailsPosition] = useState('right'); // 'right' or 'left'
...
const [sidebarView, setSidebarView] = useState('participants'); // 'participants' or 'chat'
  // अन्य state variables के साथ add करें
const [isWebSocketInitialized, setIsWebSocketInitialized] = useState(false);

// Mobile specific states
  const [isMobile, setIsMobile] = useState(false);
   const [showEmojiPicker, setShowEmojiPicker] = useState(false);
  // Add this state with other mobile states
const [showRequests, setShowRequests] = useState(false);
  const [mobileView, setMobileView] = useState('video'); // 'video', 'participants', 'chat'
  ...
  // Streamer side states
const [viewerVideoRequests, setViewerVideoRequests] = useState([]); // pending camera requests
const [viewerCameras, setViewerCameras] = useState(new Map()); // active viewer cameras
  ...
  const cleanupViewerScreenShare = (userId) => {
  setActiveScreenShare((prev) => {
    if (prev && prev.userId === userId && prev.source === "viewer-screen") {
      // Tracks band karo
      if (prev.stream) {
        try {
      ...
      addDebugLog(`🧹 Viewer screen share fully cleaned: ${userId}`);
...
const stopRecordingAPI = async () => {
  try {
    ...
    if (response.ok) {
      ...
      if (data.data) {
        toast.info(`Recording saved: ${data.data.fileName}`);
      }
      return true;
    } else {
...
const getRecordingsList = async () => {
  try {
    const response = await fetch(
      `${import.meta.env.VITE_API_URL}/api/live-session/${sessionId || roomCode}/recordings`,
      {
        headers: {
          'Authorization': `Bearer ${token}`
        }
...
const handleRecordingToggle = async () => {
  ...
  const handleStartRecording = () => {
    // सीधे recording start करने के लिए prop pass करें
    setIsRecording(true);
...
// Jab bhi screen share active हो जाए, automatically zoom करो
useEffect(() => {
...
// ✅ SIMPLE USER INTERACTION DETECTOR
useEffect(() => {
  const handleFirstUserInteraction = () => {
    ...
    // Play all pending and existing audio
    playAllAudio();
  ...
  // Agar page load pe koi audio pending hai to modal show karo
  if (!userInteracted) {
...
const playAllAudio = () => {
  ...
  setTimeout(() => {
    ...
    // Create and play all pending audio streams
...
const createAndPlayAudioElement = (userId, audioStream, userName) => {
  try {
    // Pehle existing audio element check karo
    let audioEl = document.getElementById(`viewer-audio-${userId}`);
    
    // Agar audio element already hai aur playing hai to usko update karo
    if (audioEl) {
    ...
    // Audio play with better error handling
    const playAudio = () => {
      if (audioEl.paused) {
        audioEl.play()
          .then(() => {
    ...
    // Agar user already interacted hai to direct play karo
    if (userInteracted) {
...
// ✅ SIMPLIFIED AUDIO CONSUMER HANDLER
const handleAudioConsumer = (audioTrack, producerInfo, sourceType) => {
  ...
  if (userInteracted) {
    // User already interacted - directly create and play
    createAndPlayAudioElement(userId, audioStream, userName);
  } else {
    // User hasn't interacted yet - queue the audio
  ...
  try {
    ...
    socket.emit(
      ...
      async (info) => {
        ...
        // ✅ FIX: viewer-screen-audio के लिए अलग हैंडलिंग
        ...
        // ✅ यदि viewer का audio है, तो user id check न करें
        if (isViewerSource) {
          addDebugLog(`🎤 Processing viewer audio (${info.source}) from user ${info.userId}`);
          // Continue without self-check
        } else {
...
const cleanupViewerAudio = (userId) => {
  ...
  // ✅ Step 1: Remove from pending audio queue FIRST (before any DOM operations)
  ...
  // ✅ Step 2: Clean up audio elements from refs and DOM
  ...
  // ✅ Step 4: Clean up DOM elements (including backup cleanup)
  cleanupAudioElement();
  
  // Backup cleanup - check for any remaining audio elements in DOM
  setTimeout(() => {
...
return (
  <>
    ...
    {isMobile ? (
      <div className="flex flex-col h-screen bg-black text-white overflow-hidden">
        {/* Mobile Header */}
        <div className={`bg-gray-800/90 backdrop-blur-md p-3 flex justify-between items-center border-b border-gray-600 transition-all duration-300 ${
        ...
        {mobileView === 'video' && (
          ...
          >
            ...
            {zoomed ? (
              <div className="relative w-full h-full">
                ...
                <button
                  ...
                  className="absolute top-3 right-3 bg-black/70 text-white p-1 rounded"
                >
                  <FiX className="h-4 w-4" />
                </button>
              </div>
            ) : (
              // Default streamer video
              <video
                ref={videoRef}
                autoPlay
                playsInline
            ...
            {showThumbnails && (
              <div className="absolute bottom-20 left-0 right-0 bg-gray-800/90 backdrop-blur-md p-3 max-h-40 overflow-y-auto">
            ...
            <div className="space-y-2 p-4">
              {participants.map((participant, index) => (
                ...
                >
                  ...
                  <div className="flex space-x-1">
                    {participant.hasAudio && (
                      <FiMic className="h-4 w-4 text-blue-400" />
                    )}
                ...
                messages.map((message, index) => (
                  <div
                    ...
                    className={`flex flex-col max-w-[90%] ${
                      message.userId === user?.id ? "ml-auto items-end" : "mr-auto items-start"
                    }`}
                  >
        ...
        {/* Mobile Navigation */}
        <div className={`bg-gray-800/90 backdrop-blur-md border-t border-gray-600 transition-all duration-300 ${
          showMobileControls ? 'opacity-100' : 'opacity-0'
        }`}>
          <div className="flex justify-around p-2">
            ...
            <button
              ...
              className={`flex flex-col items-center p-2 rounded-lg transition-all ${
                showThumbnails ? 'bg-purple-600 text-white' : 'text-gray-400'
              }`}
            >
              <FiGrid className="h-5 w-5" />
              <span className="text-xs mt-1">Thumbs</span>
            </button>
          </div>
        </div>

        {/* Mobile Controls */}
        <div className={`bg-gray-800/90 backdrop-blur-md p-3 transition-all duration-300 ${
          showMobileControls ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
        }`}>
      ...
      /* Desktop View - Your existing desktop code */
...
<ProducerStatus producersState={producersState} />
  ...
  <div className="flex justify-center space-x-6">
    ...
    {/* ✅ WHITEBOARD BUTTON - यहां Add करें */}
    {/* <button
...
{/* Recording Button - Updated */}
<button
  onClick={handleRecordingToggle}
  disabled={isRecordingLoading || isRecordingStopping} // ✅ Dono cases mein disable
  className={`flex flex-col items-center p-4 rounded-2xl focus:outline-none transition-all duration-200 transform hover:scale-110 shadow-lg ${
    isRecording 
...
>
  ...
  ) : isRecordingStopping ? ( // ✅ Stop होने पर loader
    <>
      <FiLoader className="text-xl mb-1 animate-spin" />
...
{/* {showWhiteboard && (
  <WhiteboardComponent
    // ✅ ESSENTIAL PROPS
    sessionId={sessionId || ''}          // From useParams
    roomCode={roomCode || ''}            // From useParams
    wsToken={token}                      // From useAuth (WebSocket JWT token)
    sessionInfo={{
      title: session?.title || 'Live Session',
      roomCode: roomCode,
      streamerName: user?.name || 'Host',
      // Add any other session info you have
    }}
    isActive={showWhiteboard}
    onClose={() => setShowWhiteboard(false)}
    isStreamer={true}                    // Since this is streamer view
    
    // ✅ NEW PROPS BASED ON YOUR SERVER
    allowViewersToDraw={true}            // Default true, you can make this configurable
  />
)} */}
      </div>
    )}
...
//         // ✅ STRICT CHECK: Streamer apna audio/video consume na kare
//         const isSelfProducer = info.userId?.toString() === user?.id?.toString();
//         const isStreamerAudio = info.source === 'streamer-mic' || info.source === 'mic' || info.source === 'screen-audio';
//         const isStreamerCamera = info.source === 'streamer-camera' || info.source === 'camera';
...
//                 kind: consumerParams.kind,
//                 rtpParameters: consumerParams.rtpParameters,
//                 appData: { source: info.source || "camera" },
//               });
...
//                     userName:
//                       info.userName ||
//                       (info.source === "screen" ? "Streamer" : "Viewer"),
//                     stream: screenStream,
//                     source: info.source,
//                   });
//                   addDebugLog(`🖥️ Active screen set (${info.source})`);

//                 } else if (info.source === "screen-audio" || info.source === "viewer-screen-audio") {
//                   // 🔹 Screen share AUDIO - Simplified permission system
//                   handleAudioConsumer(consumer.track, info, "screen-audio");
...
//   const socketRef = useRef(null);
//   const [device] = useState(new mediasoupClient.Device());
//   const [previousZoom, setPreviousZoom] = useState(null); // ✅ यह ADD करें
...
//   // ✅ Agar current zoomed screen share ही stop हो रहा है
//   if (zoomed && zoomed.type === "screen" && zoomed.userId === user.id) {
//     // Previous zoom restore करें
//     if (previousZoom) {
//       setZoomed(previousZoom);
//       setPreviousZoom(null);
//       addDebugLog(`↩️ Streamer screen share stopped, restoring previous zoom`);
//     } else {
//       // Streamer camera show करें
//       if (mediaStream) {
//         setZoomed({
//           type: "streamer",
//           stream: mediaStream,
...
//           userName: "You"
//         });
//         addDebugLog(`↩️ Showing streamer camera`);
//       } else {
//         setZoomed(null);
//       }
//     }
...
//     addDebugLog("📤 Emitted screen-share-stop to server");
//   }
// }, [socket, sessionId, user, activeScreenShare, zoomed, previousZoom, mediaStream]);
...
//   // ✅ CURRENT zoom को previous में save करें (अगर screen share नहीं है)
//   if (zoomed && zoomed.type !== "screen") {
//     setPreviousZoom(zoomed);
//     addDebugLog(`📝 Saving previous zoom: ${zoomed.type} from ${zoomed.userId}`);
//   }
...
//   addDebugLog(`🔄 Auto-zooming to screen share from ${data.userName}`);
// }, [zoomed, addDebugLog]);

// const handleScreenShareStopped = useCallback((data) => {
//   addDebugLog(`🛑 Screen share stopped by viewer: ${data.userId}`);

//   // ✅ अगर current zoomed screen share ही stop हो रहा है
//   if (zoomed && zoomed.type === "screen" && zoomed.userId === data.userId) {
//     // Previous zoom restore करें
//     if (previousZoom) {
//       setZoomed(previousZoom);
//       setPreviousZoom(null); // Clear previous
//       addDebugLog(`↩️ Restoring previous zoom: ${previousZoom.type} from ${previousZoom.userId}`);
//     } else {
//       // No previous zoom, तो streamer camera show करें
//       if (mediaStream) {
//         setZoomed({
//           type: "streamer",
//           stream: mediaStream,
...
//           userName: "You"
//         });
//         addDebugLog(`↩️ No previous zoom, showing streamer camera`);
//       } else {
//         setZoomed(null);
//       }
//     }
...
//     screenRef.current.srcObject = null;
//   }
// }, [zoomed, previousZoom, mediaStream, user, screenRef]);
...
//                   setActiveScreenShare(screenShareObj);
//                   addDebugLog(`🖥️ Active screen set (${info.source})`);
                  
//                   // ✅ Agar zoomed screen share है तो उसका stream update करें
//                   if (zoomed && zoomed.type === "screen" && zoomed.userId === info.userId) {
//                     setZoomed(prev => ({
//                       ...prev,
//                       stream: screenStream
//                     }));
//                     addDebugLog(`✅ Updated zoomed screen share with actual stream`);
//                   }
...
//     // ✅ CURRENT zoom save करें (अगर screen share नहीं है)
//     if (zoomed && zoomed.type !== "screen") {
//       setPreviousZoom(zoomed);
//       addDebugLog(`📝 Saving previous zoom: ${zoomed.type}`);
//     }

//     // ✅ Set active screen share state
//     const screenShareObj = {
//       userId: user.id,
//       userName: user.name || "Streamer",
//       stream: stream,
...
//     addDebugLog(`🔄 Auto-zooming to streamer's screen share`);

//     // ... बाकी का existing code वैसा ही रहने दें ...
    
//   } catch (error) {
//     console.error("❌ Error starting screen share:", error);
//   }
// }, [user, sendTransportRef, stopScreenShare, socket, sessionId, roomCode, zoomed]);
...
//   // ✅ REMOVED: viewer requests states
//   // const [viewerAudioRequests, setViewerAudioRequests] = useState([]); // REMOVED
//   // const [screenShareRequests, setScreenShareRequests] = useState([]); // REMOVED
//   // const [viewerVideoRequests, setViewerVideoRequests] = useState([]); // REMOVED
...
//   // ✅ REMOVED: handleViewerAudioRequest function
//   // const handleViewerAudioRequest = useCallback((data) => {
//   //   addDebugLog(`Viewer audio request from: ${data.requestedUserId}`);
//   //   setViewerAudioRequests(prev => [...prev, {
//   //     requestedUserId: data.requestedUserId,
...
//       if (audioEl.paused) {
//         audioEl.play().catch(err => {
//           addDebugLog(`❌ Failed to play audio for ${userId}: ${err.message}`);
//         });
//       }
//     });
...
//     addDebugLog('✅ All audio enabled and playing');
//   };
...
//           addDebugLog(`✅ Audio playing for user ${userId}`);
//         }).catch(error => {
//           addDebugLog(`❌ Audio play failed for ${userId}: ${error.message}`);
//         });
//       }
...
//       addDebugLog(`🎤 Audio element created for user ${userId}`);
...
//   // ✅ REMOVED: handleScreenShareRequest function
//   // const handleScreenShareRequest = useCallback((data) => {
//   //   addDebugLog(`Screen share request from: ${data.requestedUserId}`);
//   //   setScreenShareRequests(prev => [...prev, {
//   //     requestedUserId: data.requestedUserId,
...
//   // ✅ REMOVED: handleViewerAudioResponse function
//   // const handleViewerAudioResponse = useCallback((requesterSocketId, allow) => {
//   //   emitSocketEvent('viewer-audio-response', {
//   //     sessionId: sessionId || roomCode,
//   //     requesterSocketId,
...
//   // ✅ REMOVED: handleScreenShareResponse function
//   // const handleScreenShareResponse = useCallback((requesterUserId, allow) => {
//   //   emitSocketEvent('screen-share-response', {
//   //     sessionId: sessionId || roomCode,
//   //     requesterUserId,
...
//           addDebugLog(`🔄 Stopped pending audio tracks for user: ${userId}`);
//         } catch (e) {
//           addDebugLog(`⚠️ Error stopping pending audio tracks: ${e.message}`);
//         }
//       }
...
//       addDebugLog(`🗑️ Removed from pending audio queue: ${userId}`);
//     }
...
//         addDebugLog(`🛑 Stopped active audio stream for user: ${userId}`);
//       } catch (e) {
//         addDebugLog(`⚠️ Error stopping active audio stream: ${e.message}`);
//       }
//       viewerAudiosRef.current.delete(userId);
...
//         addDebugLog(`⏸️ Paused audio element for user: ${userId}`);
//       } catch(e) {
//         addDebugLog(`⚠️ Error pausing audio element: ${e.message}`);
//       }
//       audioEl.srcObject = null;
...
//         }
//       } catch (e) {
//         addDebugLog(`⚠️ Error removing audio element from DOM: ${e.message}`);
//       }
      
//       audioElementsRef.current.delete(userId);
//     }

//     const domAudioEl = document.getElementById(`viewer-audio-${userId}`);
//     if (domAudioEl && domAudioEl !== audioEl) {
//       try {
//         domAudioEl.pause();
//         domAudioEl.srcObject = null;
//         if (domAudioEl.parentNode) {
//           domAudioEl.parentNode.removeChild(domAudioEl);
//         }
//         addDebugLog(`🗑️ Backup DOM cleanup for audio element: ${userId}`);
//       } catch (e) {
//         addDebugLog(`⚠️ Error in backup DOM cleanup: ${e.message}`);
//       }
//     }
...
//     if (userId === activeScreenShare?.userId) {
//       const screenAudioEl = document.getElementById("viewer-screen-share-audio");
//       if (screenAudioEl) {
//         try {
//           screenAudioEl.pause();
//           screenAudioEl.srcObject = null;
...
//             addDebugLog(`✅ Streamer screen share started successfully with ${audioTrack ? 'audio' : 'no audio'}`);
//           } else {
//             addDebugLog("❌ Failed to register screen share producer");
//           }
//         }
//       );

//       videoTrack.onended = () => {
//         addDebugLog("🛑 Screen share video track ended by browser");
//         stopScreenShare();
//       };

//       if (audioTrack) {
//         audioTrack.onended = () => {
//           addDebugLog("🛑 Screen share audio track ended by browser");
//         };
//       }

//       stream.getTracks().forEach((track) => {
//         track.addEventListener("ended", () => {
//           addDebugLog(`🛑 Screen share ${track.kind} track ended unexpectedly`);
//           if (track.kind === "video") {
//             stopScreenShare();
//           }
//         });
...
//       toast.success(`🖥️ Screen sharing started! ${audioTrack ? '(with audio)' : '(video only)'}`, {
//         position: "bottom-right",
//         autoClose: 3000,
//       });
...
//       if (error.name === "NotAllowedError") {
//         errorMessage = "Screen share permission denied by browser";
//       } else if (error.name === "NotFoundError") {
//         errorMessage = "No screen share source found";
//       } else if (error.name === "NotSupportedError") {
//         errorMessage = "Screen sharing not supported on this device/browser";
//       }
...
//       });
//     }
//   }, [user, sendTransportRef, stopScreenShare, socket, sessionId, roomCode]);
...
//     // ========== SOCKET EVENT LISTENERS ==========
//     // ✅ REMOVED: viewer-audio-request listener
//     // ✅ REMOVED: screen-share-request listener
//     // ✅ REMOVED: viewer-video-request listener
...
//   // ✅ REMOVED: handleViewerVideoRequest function
//   // const handleViewerVideoRequest = useCallback((data) => {
//   //   addDebugLog(`📷 Viewer requested camera: ${data.requesterName} (${data.requestedUserId})`);
...
//   // ✅ REMOVED: handleViewerVideoResponse function
//   // const handleViewerVideoResponse = useCallback((requesterSocketId, allow) => {
//   //   if (!socketRef.current || !socketRef.current.connected) {
//   //     addDebugLog('❌ Socket not connected — cannot send viewer-video-response');
//   //     return;
