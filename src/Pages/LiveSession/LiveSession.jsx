...
// //recording from frontend side and screen sharing and recording are with two different streams
...
// const recordingCaptureStreamRef = useRef(null); // Sirf Recording ke liye
// const sharingCaptureStreamRef = useRef(null);   // Sirf Viewers ko dikhane ke liye
...
//     addDebugLog("🎬 Requesting dedicated screen capture for Recording...");

//     // Independent stream for recording (pure session ko record karne ke liye)
//     const stream = await navigator.mediaDevices.getDisplayMedia({
//       video: { 
//         cursor: "always",
//         frameRate: { ideal: 30 } 
...
//     recordingCaptureStreamRef.current = stream;

//     // Browser ke "Stop Sharing" (Recording wali window) ka handler
//     stream.getVideoTracks()[0].onended = () => {
//       addDebugLog("🛑 Recording capture stream ended by user");
//       if (isRecordingRef.current) {
//         stopRecording();
//       }
//     };

//     addDebugLog("✅ Dedicated recording stream obtained.");
    
//     // Yahan humne direct stream bhej di hai
//     await startRecordingFromScreenShare(stream);

//   } catch (error) {
//     console.error("Failed to start recording:", error);
//     toast.error("Recording permission denied or failed");
...
//   addDebugLog("🛑 Stopping Screen Share (Audience only)...");

//   // Producers band karein
...
//       if (val.source === 'screen' || val.source === 'screen-audio') m.delete(key);
//     });
//     return m;
//   });

//   // Sharing wali stream ko stop karein
//   if (sharingCaptureStreamRef.current) {
//     sharingCaptureStreamRef.current.getTracks().forEach(t => t.stop());
//     sharingCaptureStreamRef.current = null;
//   }

//   setActiveScreenShare(null);
//   setZoomed(prev => (prev?.type === "screen" ? null : prev));
  
//   // Note: recordingCaptureStreamRef ko touch nahi kiya gaya hai, isliye recording chalti rahegi.
...
//   try {
//     // 1. Agar pehle se sharing ON hai, toh stop karein
//     if (activeScreenShareRef.current?.source === "streamer") {
//       stopScreenShare();
//       return;
//     }

//     addDebugLog("🖥️ Requesting independent screen capture for Viewers...");

//     // 2. Direct naya capture mangna (Recording stream se alag)
...
//         height: { ideal: 720 },
//       },
//       audio: true, // Audience ko system audio sunane ke liye
//     });

//     // 3. Is stream ko separate ref mein save karein
//     sharingCaptureStreamRef.current = shareStream;

//     // 4. Jab user browser ke "Stop Sharing" button se band kare
//     const videoTrack = shareStream.getVideoTracks()[0];
//     if (videoTrack) {
//       videoTrack.onended = () => {
//         addDebugLog("🛑 Viewers screen share ended via Browser UI");
//         stopScreenShare();
//       };
//     }

//     // 5. Participants ko stream bhejna (Mediasoup Producers start honge)
//     await startScreenShareForParticipants(shareStream);

//     // ✅ Naya Trigger: Echo sync modal (Aapka existing logic)
//     setShowEchoSync(true); 

//     // ✅ Safety: Local playback mute karna taaki streamer ko khud ki awaaz na gunje
...
//     // 6. UI Update
//     setZoomed({ type: "screen", stream: shareStream, userId: user?.id });
//     toast.success("Screen shared with participants");

//   } catch (e) {
//     console.error("handleStreamerScreenShareClick error:", e);
//     // User ne cancel kiya toh error na dikhayein, sirf log karein
