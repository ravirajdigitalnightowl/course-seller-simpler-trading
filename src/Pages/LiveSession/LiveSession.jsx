...
// const recordingAudioNodesRef = useRef(new Map()); 
// // key => { sourceNode, gainNode }
...
// const ensureRecordingMixer = async () => {
//   if (recordingAudioContextRef.current && recordingAudioDestinationRef.current) return;

//   const AC = window.AudioContext || window.webkitAudioContext;
//   const ac = new AC();
//   const dest = ac.createMediaStreamDestination();

//   recordingAudioContextRef.current = ac;
//   recordingAudioDestinationRef.current = dest;

//   // map clear
//   recordingAudioNodesRef.current = new Map();
// };
// const addAudioToRecording = (key, stream, gainVal = 1.0) => {
//   const ac = recordingAudioContextRef.current;
//   const dest = recordingAudioDestinationRef.current;
//   const map = recordingAudioNodesRef.current;
...
//   map.set(key, { sourceNode, gainNode });
// };

// const removeAudioFromRecording = (key) => {
//   const map = recordingAudioNodesRef.current;
...
//     // ✅ 0) Ensure recording mixer is ready (persistent for dynamic joins/leaves)
//     await ensureRecordingMixer();
...
//     // 2) Prepare Recording Stream
//     const recordingStream = new MediaStream();

//     // ✅ Add Video (original screen video track)
//     const videoTrack = screenShareStream?.getVideoTracks?.()[0];
...
//     // ✅ ContentHint: Screen audio track (if exists)
//     const screenAudioTrack = screenShareStream?.getAudioTracks?.()[0];
//     if (screenAudioTrack && "contentHint" in screenAudioTrack) {
//       screenAudioTrack.contentHint = "music";
...
//     // 3) ✅ Add all audio sources into the MIXER (Streamer + Screen + Viewers)
//     // 3.1 Streamer mic
//     if (currentMicStream?.getAudioTracks?.().length) {
//       // mic stream ke track pe hint
...
//       addAudioToRecording("streamer-mic", currentMicStream, 1.0);
//     }

//     // 3.2 Streamer screen/system audio (agar present)
//     if (screenShareStream?.getAudioTracks?.().length) {
//       addAudioToRecording("screen-audio", screenShareStream, 0.6);
//     }

//     // 3.3 Viewers already present (mic + viewer-screen-audio)
//     // viewerAudiosRef.current keys should be like:
//     //   viewer-mic-<userId>
//     //   viewer-screen-audio-<userId>
//     for (const [key, stream] of (viewerAudiosRef.current?.entries?.() || [])) {
//       if (!stream?.getAudioTracks?.().length) continue;

//       const isViewerScreenAudio = key.startsWith("viewer-screen-audio-");
//       addAudioToRecording(key, stream, isViewerScreenAudio ? 0.7 : 0.85);
//     }

//     // 4) ✅ Add the MIXED audio track (single track) to recording stream
//     const mixedAudioTrack =
//       recordingAudioDestinationRef.current?.stream?.getAudioTracks?.()[0] || null;

//     if (mixedAudioTrack) {
//       recordingStream.addTrack(mixedAudioTrack);
//       addDebugLog("🎛️ Added mixed audio track from recording mixer");
//     } else {
//       // fallback: mic direct (rare case)
//       const fallbackMicTrack = currentMicStream?.getAudioTracks?.()[0];
//       if (fallbackMicTrack) {
//         recordingStream.addTrack(fallbackMicTrack);
//         addDebugLog("⚠️ Mixer missing: Added mic track directly as fallback");
//       } else {
//         addDebugLog("⚠️ No audio track available for recording");
//       }
//     }
...
//     // ✅ mark recording ref OFF early (so new consumers won't try to add)
...
//     // 3) ✅ NEW: Recording mixer cleanup (disconnect nodes + close context)
//     try {
//       // disconnect all nodes safely
//       if (recordingAudioNodesRef?.current) {
//         for (const [key, item] of recordingAudioNodesRef.current.entries()) {
//           try { item.sourceNode?.disconnect?.(); } catch {}
//           try { item.gainNode?.disconnect?.(); } catch {}
//         }
//         recordingAudioNodesRef.current.clear();
//       }

//       // close recording audio context
//       if (recordingAudioContextRef?.current) {
//         try { await recordingAudioContextRef.current.close(); } catch {}
//         recordingAudioContextRef.current = null;
//         recordingAudioDestinationRef.current = null;
//       }
//     } catch (e) {
//       console.warn("Recording mixer cleanup failed:", e);
//     }

//     // 4) ✅ If you still have old refs (optional safety)
//     // (If you fully migrated to recordingAudioContextRef, you can remove this block)
//     try {
//       if (audioContextRef?.current) {
//         await audioContextRef.current.close();
//         audioContextRef.current = null;
...
//     // 5) Screen capture cleanup:
//     // ✅ IMPORTANT: stopScreenShare() yahan CALL MAT KARO
//     // warna producer close karke tracks/logic side-effects aa sakte hain.
//     // Recording stop ke baad:
//     // - Agar active screen share chal raha hai, user alag se stop kare.
//     // - Agar screen share nahi hai aur screenCaptureActive true hai, cleanup karo.
//     if (!activeScreenShare && screenCaptureActive) {
//       addDebugLog("🧹 Cleaning up screen capture (no active screen share)");
//       cleanupScreenCapture();
//     }
...
//     // Backward compatibility (mic-only map)
...
//   // =========================================================
//   // ✅ RECORDING (UPDATED): add viewer-mic + viewer-screen-audio
//   //     using the NEW recording mixer helpers
//   // =========================================================
//   if (isRecordingRef.current) {
//     try {
//       // Only these two should be mixed dynamically
//       if (sourceType === "viewer-mic" || sourceType === "viewer-screen-audio") {
//         // Key must match the same key used elsewhere
//         const mixKey = `${sourceType}-${userId}`; // e.g. viewer-screen-audio-123
//         const gainVal = sourceType === "viewer-mic" ? 0.85 : 0.7;

//         // Ensure mixer exists (safe even if already created)
//         // NOTE: ensureRecordingMixer is async; but mixer should already exist once recording started.
//         // Still, we keep it safe:
//         if (!recordingAudioContextRef.current || !recordingAudioDestinationRef.current) {
//           // if for some reason mixer wasn't initialized, init now
//           // (no await to keep function non-async; but you CAN make handleAudioConsumer async if you want)
//           // We'll just skip if not ready:
//           addDebugLog("⚠️ Recording mixer not ready yet; skipping dynamic add");
//           return;
//         }

//         addAudioToRecording(mixKey, audioStream, gainVal);

//         addDebugLog(
//           `🎛️ Added to recording mixer: ${mixKey} (gain=${gainVal})`
//         );
//       }
//     } catch (err) {
//       console.warn(`Failed to add ${sourceType} to recording mixer:`, err);
//       addDebugLog(`⚠️ Failed to add ${sourceType} to recording mixer: ${err.message}`);
//     }
//   }
// };
...
//   // ✅ NEW: remove from recording mixer (dynamic)
//   const removeFromRecordingMixer = (key) => {
//     try {
//       // our new mixer map uses: recordingAudioNodesRef.current
//       removeAudioFromRecording(key);

//       addDebugLog(`🎛️ Removed from recording mixer: ${key}`);
//     } catch (e) {
//       console.warn("removeFromRecordingMixer error:", e);
//     }
//   };
...
//     // ✅ Remove stray audio elements (legacy)
//     removeAudioElById(`audio-${data.userId}-screen-audio`);
//     removeAudioElById(`audio-${data.userId}-screen`);

//     // ✅ Also clear pending queue entries
//     try {
//       pendingAudioQueueRef.current.delete(`screen-${data.userId}`);
//       pendingAudioQueueRef.current.delete(`viewer-screen-audio-${data.userId}`);
//       pendingAudioQueueRef.current.delete(`screen-audio-${data.userId}`);
...
//     // ✅ NEW: remove streamer screen/system audio from recording mixer when its producer closes
//     // (only screen-audio should exist as mixer key from our new code)
//     if (data.source === "screen-audio") {
//       removeFromRecordingMixer("screen-audio"); // same key used in startRecordingFromScreenShare()
//     }
...
//     // ✅ remove from recording mixer (IMPORTANT)
//     removeFromRecordingMixer(`viewer-screen-audio-${data.userId}`);

//     // Your existing cleanup (DOM element + refs)
//     cleanupScreenShareAudio(data.userId);
...
//     // ✅ remove from recording mixer (IMPORTANT)
//     removeFromRecordingMixer(`viewer-mic-${data.userId}`);
...
//     // ✅ remove from recording mixer if you used "streamer-mic" key
//     // (in our updated startRecordingFromScreenShare we used "streamer-mic")
//     removeFromRecordingMixer("streamer-mic");
