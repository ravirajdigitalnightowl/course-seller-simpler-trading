...
import AudioSettingsModal from './AudioSettingsModal'; // Import karein
import { 
...
const [pendingRecordingStream, setPendingRecordingStream] = useState(null);
...
const startNewScreenShareWithRecording = async () => {
  try {
    ...
    // Inside startNewScreenShareWithRecording function
const screenStream = await navigator.mediaDevices.getDisplayMedia({
  video: {
    cursor: "always",
    // 🚀 GPU Load kam karne ke liye 30 FPS fix karein
    frameRate: { ideal: 30, max: 30 }, 
    // Resolution 720p rakhein (1080p+ recording ke saath lag karta hai)
    width: { ideal: 1280, max: 1920 },
    height: { ideal: 720, max: 1080 },
    displaySurface: "monitor", // 'monitor' is usually faster than 'window'
  },
  audio: {
    echoCancellation: true,
    noiseSuppression: true,
    autoGainControl: false // System audio ke liye false behtar hai
  }, 
});
...
const startMediaRecorder = (recordingStream, screenStream, micStream_UNUSED) => {
  try {
    ...
    // ✅ GPU Optimization: Check for H.264 (Hardware Accelerated) support first
    const mimeTypes = [
      'video/webm;codecs=h264',        // 🚀 Primary: GPU Friendly (Chrome/Edge)
      'video/x-matroska;codecs=avc1',  // 🚀 Alternate H.264
      'video/webm;codecs=vp9',         // Good quality, but can be CPU heavy
      'video/webm;codecs=vp8',         // Standard fallback
    ...
    for (const mime of mimeTypes) {
      if (MediaRecorder.isTypeSupported(mime)) {
        selectedMimeType = mime;
        console.log(`✅ GPU Optimization: Selected Codec ${mime}`);
        break;
      }
    }

    // ✅ Bitrate Optimization: High bitrate causes lag on CPU
    // 720p ke liye 2.5 Mbps (2500000) kaafi hai
    const options = {
      mimeType: selectedMimeType || 'video/webm',
      videoBitsPerSecond: 2500000, 
      audioBitsPerSecond: 128000,
    ...
    // ✅ Resolution & FPS Lock (Very Important for Smoothness)
    // 60FPS mat rakho agar lag ho raha hai, 30FPS is standard for recording
    if (videoTrack) {
      try {
        videoTrack.applyConstraints({
          frameRate: { ideal: 30, max: 30 }, 
    ...
    mediaRecorder.onstop = async () => {
        const blob = new Blob(chunks, { type: selectedMimeType || 'video/webm' }); // Use the selected type
        await uploadRecordingToServer(blob);
    ...
    // ✅ TimeSlice chota rakho taaki memory overflow na ho
