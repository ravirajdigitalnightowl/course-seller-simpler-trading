...
const confirmRemoveAudio = () => {
  ...
  // ✅ PLAY ALL AUDIO (existing + pending)
const playAllAudio = () => {
...
const createAndPlayAudioElement = (userId, audioStream, userName) => {
  try {
    ...
    // Play audio if user has interacted
    if (userInteracted) {
      audioEl.play().then(() => {
...
// ✅ MANUALLY ENABLE AUDIO (modal button)
...
const cleanupViewerAudio = (userId) => {
  ...
  // ✅ Remove from pending audio queue
  ...
  // ✅ Stop and remove from viewerAudiosRef
  ...
  // ✅ Remove from audioElementsRef and DOM
  ...
  // ✅ Also remove by ID (backup cleanup)
  ...
  // ✅ Remove screen share audio if exists
  ...
  // ✅ Check if we need to hide the audio modal (if no more pending audio)
...
const startScreenShare = useCallback(async () => {
  try {
    ...
    const audioTrack = stream.getAudioTracks()[0];  // ✅ Audio track get karo
    ...
    // ✅ AUDIO PRODUCER BANAO (IF AUDIO AVAILABLE)
    ...
    // ✅ Notify server about video producer
    socket.emit(
      ...
      (response) => {
        if (response && response.id) {
          ...
          // ✅ Notify server about audio producer (IF AVAILABLE)
          if (audioProducer) {
          ...
          socket.emit("screen-share-started", {
            ...
            userId: user.id,
            userName: user.name || "Streamer",
            hasAudio: !!audioTrack,  // ✅ Batayega ki audio hai ya nahi
            source: "screen"
          });
    ...
    // ✅ Handle browser dialog close for all tracks
    stream.getTracks().forEach((track) => {
...
newSocket.on("producer-closed", (data) => {
  ...
  createConsumer(sessionId || roomCode, data.producerId, data.kind);
    ...
    // 🔥 Full snapshot event (backend se puri participants list aayegi)
